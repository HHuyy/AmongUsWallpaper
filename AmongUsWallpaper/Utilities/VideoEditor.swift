...
class VideoEditors {
    ...
    private func add(text: String, y: CGFloat, textStyle: String, textColor: UIColor, textSize: CGFloat, icon: UIImage, to layer: CALayer, videoSize: CGSize, alignment: TextAlignment, scaleHeight: CGFloat, scaleWidth: CGFloat, textScaleHeight: CGFloat, textScaleWidth: CGFloat, backgroundColor: UInt) {
        ...
        let attributedText = NSAttributedString(string: text, attributes: [
                                                    .font: UIFont(name: textStyle, size: textSize * scaleWidth) as Any,
        ...
        let textLayerHeight = attributedText.height(containerWidth: videoSize.width * textScaleWidth)
        
        textLayer.frame = CGRect(
            x: 10 * scaleWidth,
            y: mainY - (videoSize.height * textScaleHeight)/2,
        ...
        gradient.frame = CGRect(
            ...
            y: mainY - (videoSize.height * textScaleHeight)/2,
            width: videoSize.width * 1.2,
            height: videoSize.height * textScaleHeight)
        
        let textAnimation = CABasicAnimation(keyPath: "position")
        textAnimation.fromValue = [videoSize.width/2 - imageHeight * (2/3), textLayer.frame.maxY - textLayer.frame.height/2] //attributedText.width(containerHeight: videoSize.height)*2
        textAnimation.toValue = [videoSize.width*2, textLayer.frame.maxY - textLayer.frame.height/2]
        ...
        let crewMoveAnimation = CABasicAnimation(keyPath: "position")
        crewMoveAnimation.fromValue = [ -crewIconLayer.frame.width, textLayer.frame.maxY - textLayer.frame.height/2] //attributedText.height(containerWidth: videoSize.width)
        crewMoveAnimation.toValue = [videoSize.width + crewIconLayer.frame.width, textLayer.frame.maxY - textLayer.frame.height/2]
