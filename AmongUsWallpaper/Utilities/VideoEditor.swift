...
class VideoEditors {
    func makeVideo(fromVideoAt videoURL: URL, icon: UIImage, fontStyle: String, textString text: String, textColor: UInt, background: UIImage, textSize: CGFloat, alignment: TextAlignment, y: CGFloat, scale: CGFloat, onComplete: @escaping (URL?) -> Void) {
//    onComplete(videoURL)
...
//    outputLayer.addSublayer(backgroundLayer)
  ...
  private func addImage(to layer: CALayer, videoSize: CGSize) {
    ...
    private func add(text: String, y: CGFloat, textStyle: String, textColor: UIColor, textSize: CGFloat, icon: UIImage, to layer: CALayer, videoSize: CGSize, alignment: TextAlignment, scale: CGFloat) {
